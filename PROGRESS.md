...
- Verified locally on 2026-06-10: `cargo test` passes across the workspace including the new `netweevil-simulate` unit and synthetic-grid integration tests (deterministic seeded runs, congestion platoon delay, forced reroute around a no-access zone, simultaneous car+bicycle fleets).
- Verified locally on 2026-06-10: a live API smoke test on `north_nl_2026_05_10` ran 2000 cars + 500 bicycles simultaneously (zero dispatch failures), streamed frames and congestion bins during execution, applied pause/resume, a global speed factor, and a mid-run car no-access zone (2164 live reroutes), and served busy segments, per-agent trajectories, and temporal GeoJSON after completion. Simulation runs are reproducible for a fixed scenario seed.
- Simulation dispatch is batched: every unique endpoint is snapped once via `PreparedRoutingEngine::snap_route_candidates` and every unique (origin, destination, alternatives) pair is routed once via `execute_route_between_candidates`, then shared across agents (each agent still picks its own ranked alternative deterministically). Verified locally on 2026-06-10 against `north_nl_2026_05_10`: 2000 agents from 10 weighted origin points to 2 destination points dispatch in ~2s (~30x faster than per-agent routing), and 2500 fully random agents without alternative routes dispatch in ~9s; high `alternative_route_share` with fully random demand remains the documented expensive path since every alternative search is a unique pair.
...
- [x] Add `netweevil-simulate`: a deterministic mesoscopic agent-based traffic simulation crate on the shared edge graph, with multi-mode fleets (any loaded profiles simultaneously), configurable origin/destination distributions (dataset bounds, bbox, polygon, weighted points, named zones), departure models (uniform/peak/instant/Poisson), per-agent behavior parameters (overtake eagerness, reroute eagerness, signal compliance, alternative-route share, no-collision spillback queueing, heterogeneity), polygon zones (no-access per mode, speed factor, capacity factor, standing background traffic, spawn/attract), Greenshields/BPR congestion models, fixed-cycle traffic-signal gates, live congested-cost rerouting with budgets/backoff, compact `(edge, enter_t)` trajectories with linear or trapezoidal accel/decel position interpolation, frame snapshots, per-edge congestion bins, and whole-run busy-segment usage totals
- [x] Add CLI `simulate validate` and `simulate run` (writes result JSON, busy-segment and congestion GeoJSON, optional temporal GeoJSON, and a `simulation` run manifest under `.netweevil/runs/`)
- [x] Add API simulation endpoints: `POST/GET /v1/simulation`, `GET/DELETE /v1/simulation/{id}`, `POST /v1/simulation/{id}/control` (pause/resume/cancel, global speed factor, add/remove zones, spawn extra fleets mid-run), `GET .../frames` (windowed columnar agent positions), `GET .../edges` (congestion bins or whole-run busy segments as GeoJSON), `GET .../temporal` (QGIS temporal-controller GeoJSON), and `GET .../agents/{id}` (trajectory GeoJSON); frames and edge statistics stream while the run is still executing
- [x] Add a QGIS plugin Simulation tab: fleet builder over API-loaded profiles, OD sources from dataset bounds/canvas extent/polygon layers/point layers, zone definition from polygon layers with live add/remove against a running simulation, run/pause/resume/cancel and global speed factor control, animated playback with frame slider and selectable playback speeds, fleet-colored agent layer, congestion-window and busy-segment styled layers, native temporal-controller export, and scenario export/import as JSON
...
- Route, OD, matrix, route-batch, and transit-route requests now support opt-in `alternatives` settings. Default requests still return one optimal route, while `max_routes > 1` adds ranked suboptimal alternatives to JSON responses and route GeoJSON features; service-area requests remain unchanged.
...
- The QGIS plugin now exposes alternative-route controls for single-route, OD/matrix batch, and transit-route requests, and loads returned alternatives as ranked route or transit summary/leg features.
